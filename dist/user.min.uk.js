...
undefined.JST = undefined.JST || {}, undefined.JST.RoleGroupEditorPopup = function (a) {
  ...
  } });i18n.addMessages({ "user_attribute_editor_popup.title": "User attribute", "user_attribute_editor_popup.button.save": "Save", "user_attribute_editor_popup.field.name": "Name", "user_attribute_editor_popup.field.description": "Description", "user_attribute_editor_popup.field.type": "Type", "user_attribute_editor_popup.field.printFormat": "Print format", "user_attribute_editor_popup.field.defaultValue": "Default value" }), i18n.addMessages({ "user_attribute_editor_popup.title": "Аттрибут пользователя", "user_attribute_editor_popup.button.save": "Сохранить", "user_attribute_editor_popup.field.name": "Название", "user_attribute_editor_popup.field.description": "Описание", "user_attribute_editor_popup.field.type": "Тип", "user_attribute_editor_popup.field.printFormat": "Формат вывода", "user_attribute_editor_popup.field.defaultValue": "Значение по умолчанию" });var UserAttributesPageView = Marionette.LayoutView.extend({ collection: null, regions: { list: ".list" }, events: { "click [data-new-attribute]": "newAttributeClickListener" }, initialize: function initialize() {
  ...
  } });i18n.addMessages({ "user_attributes_page.button.new_attribute": "New attribute", "user_attributes_page.title": "User attribute" }), i18n.addMessages({ "user_attributes_page.button.new_attribute": "Новый аттрибут", "user_attributes_page.title": "Аттрибут пользователя" });var UserEditorView = Marionette.LayoutView.extend({ regions: { content: ".content" }, initialize: function initialize() {
  ...
  } });i18n.addMessages({ "New user": "Новый пользователь", Parameters: "Параметры", Roles: "Роли", Groups: "Группы", Email: "Электронная почта", Password: "Пароль", "Password confirm": "Подтвердить пароль", Save: "Сохранить" });var UserGroupsView = Backbone.View.extend({ events: { "submit form": "saveUserGroups" }, groups: null, initialize: function initialize() {
  ...
  } });i18n.addMessages({ "user.groups.empty_list": "No groups configured" }), i18n.addMessages({ "user.groups.empty_list": "Группы отсутствуют" });var UserListView = Backbone.View.extend({ events: { "click .delete": "deleteEventListener" }, initialize: function initialize(a) {
  ...
  } });i18n.addMessages({ "user.list.edit": "Edit", "user.list.delete": "Delete", "user.list.name_not_specified": "User name not specified" }), i18n.addMessages({ "user.list.edit": "Изменить", "user.list.delete": "Удалить", "user.list.name_not_specified": "Имя не задано" });var UserParametersView = Backbone.View.extend({ events: { "submit form": "saveParametersEventHandler" }, render: function render() {
  ...
  } });i18n.addMessages({ "user_attribute_list.name": "Name", "user_attribute_list.description": "Description", "user_attribute_list.type": "Type", "user_attribute_list.printFormat": "Print Format", "user_attribute_list.defaultValue": "Default Value", "user_attribute_list.edit": "Edit" }), i18n.addMessages({ "user_attribute_list.name": "Название", "user_attribute_list.description": "Описание", "user_attribute_list.type": "Тип", "user_attribute_list.printFormat": "Формат вывода", "user_attribute_list.defaultValue": "Значение по умолчанию" });var UsersPageView = Marionette.LayoutView.extend({ regions: { list: "#users-list" }, render: function render() {
    this.$el.html(app.render("UsersPage")), this.list.show(new UserListView({ collection: new UserCollection() }));
  } });i18n.addMessages({}), i18n.addMessages({ "Add user": "Новый пользователь", Users: "Пользователи" }), !function a(b, c, d) {
  function e(g, h) {
    if (!c[g]) {
      if (!b[g]) {
